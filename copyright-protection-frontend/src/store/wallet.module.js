...
const actions = {
  async initWeb3Modal({ commit }) {
    ...
    if (localStorage.getItem("isConnected") === "true") {
      const providerW3m = await web3Modal.connect();
  ...
  async ethereumListener({ commit }) {
    window.ethereum.on("accountsChanged", (accounts) => {
      if (state.isConnected) {
        const provider = new ethers.providers.Web3Provider(state.providerW3m);
  ...
  async fetchActiveBalance({ commit }) {
    const balance_eth = await state.providerW3m.getBalance(state.activeAccount);
    const balance_wei = ethers.utils.formatEther(balance_eth);
